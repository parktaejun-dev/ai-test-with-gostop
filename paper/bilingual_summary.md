...
이 문서는 공개 원고 패키지의 읽기 순서를 고정한다. 모든 핵심 설명은 한국어를 먼저 제공하고, 바로 뒤에 영어를 보조 언어로 제공한다.

This note fixes the reading order for the public manuscript package. Korean is provided first, followed by English as the secondary language.
...
## 산출물 안내 | Artifact Reminder

한국어:
- `paper/arxiv_main.tex`: 영어 학술 원고 소스.
- `paper/gostop_ai_evaluation_paper.pdf`: 한국어 우선, 영어 보조 공개 PDF.
- `dashboard/paper.html`: 한국어 우선, 영어 보조 웹 논문 페이지.
- `paper/remote_model_results.md`: 한국어 우선 결과 요약.
...
- `paper/gostop_ai_evaluation_paper.pdf`: Korean-first, English-secondary public PDF.
- `dashboard/paper.html`: Korean-first, English-secondary web paper page.
- `paper/remote_model_results.md`: Korean-first result summary.
