...
def run_player_eval(
    ...
    *,
    ...
    factories = {
        seat_name: (
            lambda mid=model_id, seat=seat_name: DashScopeModelAgent(
                name=seat,
                model_id=mid,
    ...
    bundle = export_experiment_bundle(
        output_dir,
        config={
            "experiment": "dashscope_player_eval",
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Run a fixed 4-seat DashScope lineup.")
    parser.add_argument("--seat-models", required=True, help="Comma-separated DashScope model ids for 4 seats.")
...
def main() -> None:
    ...
    output_dir = args.output_dir or _default_output_dir("dashscope_player")
